...
/*+
HashJoin(t ci n an rt chn mc cn)
NestLoop(t ci n an rt chn mc)
NestLoop(t ci n an rt chn)
NestLoop(t ci n an rt)
NestLoop(t ci n an)
NestLoop(t ci n)
NestLoop(t ci)
IndexScan(t)
...
IndexScan(mc)
SeqScan(cn)
Leading((((((((ci t) n) an) rt) chn) mc) cn))*/
...
(
	...
	WHERE pg_lip_bloom_probe(3, ci.role_id)
) AS ci ,
company_name AS cn ,
(
