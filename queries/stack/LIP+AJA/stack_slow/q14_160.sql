...
SELECT sum(pg_lip_bloom_add(0, b.site_id)), sum(pg_lip_bloom_add(1, b.user_id)), sum(pg_lip_bloom_add(2, b.site_id)), sum(pg_lip_bloom_add(3, b.user_id)) FROM badge AS b 
        WHERE ((b.name)::text = ANY ('{Analytical,Archaeologist,Booster,Cleanup,Deputy,"Documentation User",Electorate,"Great Question",Investor,"Not a Robot",Outspoken,Talkative,Taxonomist,"Unsung Hero"}'::text[])) AND ((b.name)::text = ANY ('{Analytical,Archaeologist,Booster,Cleanup,Deputy,"Documentation User",Electorate,"Great Question",Investor,"Not a Robot",Outspoken,Talkative,Taxonomist,"Unsung Hero"}'::text[])) AND ((b.name)::text = ANY ('{Analytical,Archaeologist,Booster,Cleanup,Deputy,"Documentation User",Electorate,"Great Question",Investor,"Not a Robot",Outspoken,Talkative,Taxonomist,"Unsung Hero"}'::text[])) AND ((b.name)::text = ANY ('{Analytical,Archaeologist,Booster,Cleanup,Deputy,"Documentation User",Electorate,"Great Question",Investor,"Not a Robot",Outspoken,Talkative,Taxonomist,"Unsung Hero"}'::text[]));
