...
SELECT sum(pg_lip_bloom_add(3, b.site_id)), sum(pg_lip_bloom_add(4, b.site_id)), sum(pg_lip_bloom_add(5, b.user_id)) FROM badge AS b 
        WHERE ((b.name)::text = ANY ('{Investor,"Peer Pressure",Reviewer,Tenacious,"Vox Populi"}'::text[])) AND ((b.name)::text = ANY ('{Investor,"Peer Pressure",Reviewer,Tenacious,"Vox Populi"}'::text[])) AND ((b.name)::text = ANY ('{Investor,"Peer Pressure",Reviewer,Tenacious,"Vox Populi"}'::text[]));
