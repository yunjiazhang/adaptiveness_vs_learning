...
SELECT sum(pg_lip_bloom_add(0, b.site_id)), sum(pg_lip_bloom_add(1, b.user_id)), sum(pg_lip_bloom_add(2, b.site_id)), sum(pg_lip_bloom_add(3, b.user_id)) FROM badge AS b 
        WHERE ((b.name)::text = ANY ('{Inquisitive,Mortarboard,Reviewer,"Stellar Question","Vox Populi"}'::text[])) AND ((b.name)::text = ANY ('{Inquisitive,Mortarboard,Reviewer,"Stellar Question","Vox Populi"}'::text[])) AND ((b.name)::text = ANY ('{Inquisitive,Mortarboard,Reviewer,"Stellar Question","Vox Populi"}'::text[])) AND ((b.name)::text = ANY ('{Inquisitive,Mortarboard,Reviewer,"Stellar Question","Vox Populi"}'::text[]));
