...
SELECT sum(pg_lip_bloom_add(3, b.site_id)), sum(pg_lip_bloom_add(4, b.site_id)), sum(pg_lip_bloom_add(5, b.user_id)) FROM badge AS b 
        WHERE ((b.name)::text = ANY ('{Analytical,Booster,"Copy Editor",Disciplined,Epic,Investor,Marshal,Populist,Pundit,Socratic,Synonymizer,"Tag Editor",Talkative,"Unsung Hero","Vox Populi"}'::text[])) AND ((b.name)::text = ANY ('{Analytical,Booster,"Copy Editor",Disciplined,Epic,Investor,Marshal,Populist,Pundit,Socratic,Synonymizer,"Tag Editor",Talkative,"Unsung Hero","Vox Populi"}'::text[])) AND ((b.name)::text = ANY ('{Analytical,Booster,"Copy Editor",Disciplined,Epic,Investor,Marshal,Populist,Pundit,Socratic,Synonymizer,"Tag Editor",Talkative,"Unsung Hero","Vox Populi"}'::text[]));
