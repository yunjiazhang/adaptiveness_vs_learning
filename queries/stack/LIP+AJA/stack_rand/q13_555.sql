...
SELECT sum(pg_lip_bloom_add(3, b.site_id)), sum(pg_lip_bloom_add(4, b.site_id)), sum(pg_lip_bloom_add(5, b.user_id)) FROM badge AS b 
        WHERE ((b.name)::text = ANY ('{Convention,"Copy Editor","Great Question",Inquisitive,Refiner,Socratic,Synonymizer,Taxonomist}'::text[])) AND ((b.name)::text = ANY ('{Convention,"Copy Editor","Great Question",Inquisitive,Refiner,Socratic,Synonymizer,Taxonomist}'::text[])) AND ((b.name)::text = ANY ('{Convention,"Copy Editor","Great Question",Inquisitive,Refiner,Socratic,Synonymizer,Taxonomist}'::text[]));
