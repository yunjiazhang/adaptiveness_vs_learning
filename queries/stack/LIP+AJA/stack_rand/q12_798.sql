...
WHERE 
 ...
 AND (s.site_name in ('askubuntu','serverfault','stackoverflow','tex')) 
 AND (t1.name in ('cas','commit','dotnet-httpclient','sha1','smtpclient','timespan','variadic-functions','wubi')) 
 AND (q1.score >= 0) 
 AND (q1.score <= 1000) 
 AND (u1.reputation >= 0) 
