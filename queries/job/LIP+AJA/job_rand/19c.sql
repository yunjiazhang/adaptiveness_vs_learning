...
SELECT sum(pg_lip_bloom_add(0, id)) FROM info_type AS it WHERE it.info = 'release dates';
...
FROM ( 
     ...
     (
          ...
          WHERE ci.note IN ('(voice)',
                  '(voice: Japanese version)',
                  '(voice) (uncredited)',
                  '(voice: English version)')  AND pg_lip_bloom_probe(1, role_id) -- AND pg_lip_bloom_probe(7, movie_id)
     ) AS ci,
     company_name AS cn,
     info_type AS it,
     (
     ...
     (
          ...
          AND (mi.info LIKE 'Japan:%200%'
               OR mi.info LIKE 'USA:%200%') AND pg_lip_bloom_probe(0, info_type_id)
     ) AS mi,
     name AS n,
     role_type AS rt,
     (
