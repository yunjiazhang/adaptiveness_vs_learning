...
-- SELECT sum(pg_lip_bloom_add(1, movie_id)) FROM movie_companies AS mc WHERE  mc.note NOT LIKE '%(USA)%'
...
FROM company_name AS cn,
     ...
     (
        ...
        where pg_lip_bloom_probe(4, info_type_id) 
        -- AND pg_lip_bloom_probe(1, movie_id)
      ) AS mi_idx,
     (
     ...
     (
        ...
        -- where 
        -- -- pg_lip_bloom_probe(5, kind_id) -- and 
        -- pg_lip_bloom_probe(1, id)
     ) AS t
