...
SELECT sum(pg_lip_bloom_add(2, id)) FROM company_type AS ct WHERE ct.kind = 'production companies'; -- filter on mc.company_id
SELECT sum(pg_lip_bloom_add(3, id)) FROM info_type AS it1 WHERE it1.info = 'genres'; -- filter on mc.company_id
...
FROM company_name AS cn,
    ...
    WHERE  pg_lip_bloom_probe(1, mc.company_id) AND pg_lip_bloom_probe(2, mc.company_type_id)
    AND pg_lip_bloom_probe(5, movie_id)
) AS mc,
     (
    SELECT * FROM movie_info AS mi
    WHERE mi.info IN ('Drama', 'Horror') AND pg_lip_bloom_probe(3, info_type_id) AND pg_lip_bloom_probe(5, movie_id)
) AS mi,
     (
