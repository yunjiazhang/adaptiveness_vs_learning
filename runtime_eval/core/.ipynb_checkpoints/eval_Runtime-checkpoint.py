...
class Runtime_Evaluator(object):
    ...
    def evaluate_queries(self, sql_dir=None, max_sql_num=None, sql_files=None, save_json_file=None,\
                         ...
                         disable_lip=False, disable_parallel=False, save_plan=False, disable_nestloop=False, save_plan_subdir='plan'):
        ...
        for idx, sql_file in enumerate(sql_files):
            ...
            for i in range(multiple_runs):
                ...
                # else:
                #     show_plan = False
                q = self.db_instance.execute(sql, timeout=timeout, disable_lip=disable_lip, show_plan=save_plan, disable_parallel=disable_parallel, disable_nestloop=disable_nestloop)
                end_timestamp = time.time() 
                if q['execution_cost'] < 0:
                    self.runtime_quality[sql_file] = [-1]
                    break
                else:
                    logging.info(f"\tFinished the {i+1}-th trial with {end_timestamp - start_timestamp:.2f} secs")
...
class PG_LIP_Runtime_Evaluator(Runtime_Evaluator):
    ...
    def evaluate_queries(self, eval_lip=True, sql_dir=None, sql_files=None, save_json_file=None,\
                         ...
                         disable_parallel=False, save_plan=True, disable_nestloop=False):
        ...
        for idx, sql_file in enumerate(sql_files):
            ...
            for i in range(multiple_runs):
                ...
                q = self.db_instance.connector.profile_lip_query(prepare_sqls, profile_sql, save_plan, disable_nestloop=disable_nestloop)
                end_timestamp = time.time() 
                ...
                if save_plan:
                    plan_str = '\n'.join([a[0] for a in q['plan']])
                    if disable_nestloop:
                        with open(os.path.join(sql_dir, 'plan_disable_nl' , sql_file.replace('.sql', '_plan.txt')), 'w') as f:
                            f.write(plan_str)
                    else:
